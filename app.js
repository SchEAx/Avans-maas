const CONFIG_KEY = 'garage_avans_supabase_config_v1';
...
function initSupabase(){
  ...
  if(!cfg?.url || !cfg?.key){ $('setupCard').classList.remove('hidden'); return false; }
...
function renderPeople(){
  ...
  state.people.forEach(p=>{
    ...
    div.innerHTML=`<div><b>${p.name}</b><span>Maaş: ${money.format(p.salary||0)}</span></div>`;
    const btns=document.createElement('div');
    const edit=document.createElement('button'); edit.className='small ghost'; edit.textContent='Düzenle'; edit.onclick=()=>{ $('personName').value=p.name; $('personSalary').value=p.salary||0; $('personName').dataset.editId=p.id; document.querySelector('[data-tab="tabPeople"]').click(); };
...
function bind(){
  ...
  $('saveConfigBtn').onclick=()=>{ const url=$('sbUrl').value.trim(), key=$('sbKey').value.trim(); if(!url||!key) return showToast('URL ve anon key gir knk.'); saveConfig({url,key}); initSupabase(); loadAll(); };
  $('clearConfigBtn').onclick=()=>{ localStorage.removeItem(CONFIG_KEY); location.reload(); };
  $('syncBtn').onclick=loadAll; $('savePersonBtn').onclick=savePerson; $('saveAdvanceBtn').onclick=saveAdvance; $('applyDeductionBtn').onclick=applyDeductions;
